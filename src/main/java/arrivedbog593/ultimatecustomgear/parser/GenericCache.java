...
public class GenericCache<T> {
    ...
    /**
     * Drops one entry, so the file behind it is parsed fresh next time.
     * <p>
     * Used when a cached object no longer passes validation: keeping it would
     * leave the cache holding something the mod already rejects, and the entry
     * would survive until the file was edited or deleted — removeStale only
     * clears keys whose file is gone.
     *
     * @return {@code true} if there was an entry to remove
     */
    public boolean remove(String relativePathKey) {
        return entries.remove(relativePathKey) != null;
    }
