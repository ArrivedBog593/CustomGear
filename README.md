...
- Per-piece armor effects (e.g. helmet gives Night Vision when worn)
- Set bonus effects when wearing the full armor set
- Held effects for weapons and tools (e.g. sword gives Strength when held)
- Full multi-language support for item names
- Custom textures, or reuse textures from other mods
- Compatible with JEI
...
|---|---|---|
| `id` | String | Unique identifier. Use lowercase letters, numbers, and underscores only. |
...
| `pieces.toughness` | Float | Armor toughness. Netherite = 3.0 per piece |
...
| `piece_effects` | Map | Effects applied when a specific piece is worn |
...
| `set_bonus.effects` | List | List of effects to apply |
...
| `texture.path` | String | Path relative to `.minecraft/customgear/` (for `custom` mode) |
| `texture.ref` | String | Texture resource location from another mod (for `reference` mode) |
...
### `default`
Uses the iron armor/tool texture as a placeholder. Good for testing.

### `custom`
Uses your own PNG files placed in `.minecraft/customgear/textures/`.
...
For tools, one PNG per tool type:
```
...
### `reference`
Reuses a texture from another mod already installed:
```json
"texture": {
  "mode": "reference",
  "ref": "othermod:item/some_sword"
}
```
...
CustomGear does not add crafting recipes by default. To add recipes for your custom items, use [KubeJS](https://www.curseforge.com/minecraft/mc-mods/kubejs) or a similar mod. Your item IDs follow the pattern `customgear:my_armor_helmet`, `customgear:my_tools_sword`, etc.
...
- Custom enchantments from other mods work automatically if the item is enchantable
